...
for (const result of [pages, settings, hours, media]) {
...
if (missingPages.length || missingSettings.length || (hours.data?.length ?? 0) !== 7 || (media.data?.length ?? 0) < 10) {
  throw new Error(`Seed verification failed. Missing pages: ${missingPages.join(', ') || 'none'}; missing settings: ${missingSettings.join(', ') || 'none'}; hours: ${hours.data?.length}; media: ${media.data?.length}`);
}
