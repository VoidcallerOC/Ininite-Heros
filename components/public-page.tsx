...
function CardGames({ content, games }: { content: Record<string, Json>; games: CardGame[] }) {
  return <section className="content-section content-section--paper-deep"><div className="container"><SectionHeading content={content} defaultTitle="Card games" />{games.length ? <div className="editorial-card-grid">{games.map((game, index) => <article className="editorial-card" key={game.id}>{game.image_url && <img src={game.image_url} alt={game.image_alt || `${game.name} cards at Infinite Heroes Comics`} width="720" height="480" loading="lazy" />}<span className="editorial-card__index">{String(index + 1).padStart(2, '0')}</span><h3>{game.name}</h3><p>{game.description}</p>{game.cta_label && <Link className="editorial-card__link" href={game.cta_href || '/visit.html'}>{game.cta_label} {arrow}</Link>}</article>)}</div> : <div className="admin-empty"><strong>{contentString(content, 'emptyTitle')}</strong><p>{contentString(content, 'emptyBody')}</p></div>}</div></section>;
}
