...
function Hero({ content, home = false, cards = false }: { content: Record<string, Json>; home?: boolean; cards?: boolean }) {
...
const cardGameLogoFallbacks: Record<string, string> = {
  magic: '/assets/images/card-logos/magic-event-logo.png',
...
export function PublicPage({ pageData, siteData, page, games = [], catalogSections = [], cardsContent = null }: { pageData: CmsPageData; siteData: PublicSiteData; page: string; games?: CardGame[]; catalogSections?: CatalogSection[]; cardsContent?: CardsContent | null; events?: StoreEvent[] }) {
  const sections = pageData.sections;
  const get = (key: string) => sections.find((section) => section.key === key)?.content ?? {};
  const cardsHero = cardsContent ? { ...get('hero'), eyebrow: cardsContent.eyebrow, title: cardsContent.title, body: cardsContent.description } : get('hero');
  return <SiteShell currentPage={page} data={siteData}><main id="main-content">{page === 'home' ? <><Hero content={get('hero')} home /><Announcement content={get('announcement')} /><HomeIntro content={get('intro')} /><HomeCategories content={get('categories')} /><Gallery content={get('gallery')} /><HomeOwner content={get('owner')} /><HomeVisit content={get('visit')} data={siteData} /><Cta content={get('cta')} /></> : <><Hero content={page === 'cards' ? cardsHero : get('hero')} cards={page === 'cards'} />{page === 'visit' ? <VisitDetails content={get('details')} data={siteData} /> : <>{page === 'cards' && cardsContent ? <><MagicProgramming content={cardsContent} /><CardGames content={get('games')} games={games} /></> : (page === 'comics' || page === 'collectibles') ? <CatalogSections sections={catalogSections} page={page} /> : get('owner-story').title ? <RichText content={get('owner-story')} about /> : get('features').title ? <FeatureList content={get('features')} /> : null}<Cta content={get('cta')} /></>}</>}</main></SiteShell>;
}
