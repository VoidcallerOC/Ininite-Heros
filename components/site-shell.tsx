...
export function SiteShell({ children, currentPage, data }: Readonly<{ children: React.ReactNode; currentPage: string; data: PublicSiteData }>) {
  ...
  const businessSchema = {
    ...
    '@type': 'LocalBusiness',
    name: businessName,
    url: process.env.NEXT_PUBLIC_SITE_URL || 'https://infiniteheroes.net',
    image: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://infiniteheroes.net'}${logo}`,
    ...
    openingHoursSpecification: data.hours.filter((hour) => !hour.is_closed && hour.open_time && hour.close_time).map((hour) => ({
      '@type': 'OpeningHoursSpecification',
      dayOfWeek: `https://schema.org/${hour.label}`,
